...
namespace Raven.Server.Documents.Indexes.Static.Linq
{
    public static class DynamicEnumerable
    {
        ...
        {
            if (source is DynamicArray dynamicArray)
                return dynamicArray.Except((IEnumerable)other);

            var first = (IEnumerable)source;
            var dynFirst = new DynamicArray(first.Cast<object>());

            return dynFirst.Except((IEnumerable)other);
        }
        ...
        public static dynamic Sum(IEnumerable source)
        {
            if (source == null) return DynamicNullObject.Null;
            var da = source as DynamicArray ?? new DynamicArray(source);
            return da.Sum();
        }

        public static dynamic Sum<TSource, TResult>(IEnumerable<TSource> source, Func<TSource, TResult> selector)
        {
            return Sum(Enumerable.Select(source, selector));
        }

        public static dynamic Average(IEnumerable source)
        {
            if (source == null) return DynamicNullObject.Null;
            var da = source as DynamicArray ?? new DynamicArray(source);
            return da.Average();
        ...
        public static dynamic Average<TSource, TResult>(IEnumerable<TSource> source, Func<TSource, TResult> selector)
        {
            return Average(Enumerable.Select(source, selector));
        }
