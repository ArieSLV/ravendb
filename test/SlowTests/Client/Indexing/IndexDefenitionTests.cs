...
namespace SlowTests.Client.Indexing
{
    public class IndexDefinitionTests : RavenTestBase
    {
        ...
        public void MapIndex_MemoryExtensionsContainsAny_IntArrays_ShouldWork(Options options)
        {
            var indexBuilder = new IndexDefinitionBuilder<DocWithArray, object>
            {
                Map = docs => from doc in docs
                              ...
                              {
                                  doc.Id,
                                  HasAnyNumber = doc.Numbers.ContainsAny(new[] { 42, 100 })
                              }
            };
        ...
        public void MapIndex_SequenceEqual_StringArray_ShouldWork(Options options)
        {
            var indexBuilder = new IndexDefinitionBuilder<DocWithArray, object>
            {
                Map = docs => from doc in docs
                              ...
                              {
                                  doc.Id,
                                  IsExact = doc.Tags.SequenceEqual(new[] { "news", "important" })
                              }
            };

            var docs = new object[]
            {
                new DocWithArray { Id = "items/1", Tags = new[] { "news", "important" } },
                new DocWithArray { Id = "items/2", Tags = new[] { "important", "news" } },
                new DocWithArray { Id = "items/3", Tags = new[] { "news", "sports" } }
            };
            ...
            AssertIndexBuilderRewritesAndRunsCorrectly(
                ...
                additionalRunAsserts: (store, indexName) =>
                {
                    using (var session = store.OpenSession())
                    {
                        ...
                        Assert.Single(results);
                        Assert.Equal("items/1", results[0].Id.ToString());
                    }
                });
        }
        ...
        [RavenTheory(RavenTestCategory.Indexes)]
        [RavenData(SearchEngineMode = RavenSearchEngineMode.All)]
        public void MapIndex_Contains_ULong_ShouldWork(Options options)
        {
            var indexBuilder = new IndexDefinitionBuilder<DocWithLongs, object>
            {
                Map = docs => from doc in docs
                              ...
                              {
                                  doc.Id,
                                  HasThree = doc.ULongValues.Contains(3UL)
                              }
            };

            var docs = new object[]
            {
                new DocWithLongs { ULongValues = [1UL, 2UL, 3UL] }, // Match
                new DocWithLongs { ULongValues = [1UL, 2UL] }       // No match
            };
            ...
            AssertIndexBuilderRewritesAndRunsCorrectly(
                ...
                additionalRunAsserts: (store, indexName) =>
                {
                    ...
                    {
                        var results = session.Advanced
                            .RawQuery<dynamic>($"from index '{indexName}' where HasThree = true")
                            .ToList();
        ...
        [RavenTheory(RavenTestCategory.Indexes)]
        [RavenData(SearchEngineMode = RavenSearchEngineMode.All)]
        public void MapIndex_ComplexChain_ULong_Contains_ShouldWork(Options options)
        {
            var indexBuilder = new IndexDefinitionBuilder<DocWithLongs, object>
            {
                Map = docs => from doc in docs
            ...
            var docs = new object[]
            {
                new DocWithLongs { ULongValues = [1UL, 3UL] }, // Match
                new DocWithLongs { ULongValues = [1UL] }       // No match
            };
                ...
                additionalRunAsserts: (store, indexName) =>
                {
                    using (var session = store.OpenSession())
                    {
                         var count = session.Advanced
                            .RawQuery<dynamic>($"from index '{indexName}' where Result = true")
                            .Count();
        ...
        [RavenTheory(RavenTestCategory.Indexes)]
        [RavenData(SearchEngineMode = RavenSearchEngineMode.All)]
        public void MapIndex_Concat_LeftArg_Contains_ShouldWork(Options options)
        {
            var indexBuilder = new IndexDefinitionBuilder<DocWithLongs, object>
            {
                Map = docs => from doc in docs
                              ...
                              {
                                  doc.Id,
                                  Result = new[] { 10UL }.Concat(doc.ULongValues).Contains(3UL)
                              }
            };
            ...
            {
                new DocWithLongs { ULongValues = [3UL] },
                new DocWithLongs { ULongValues = [2UL] }
            };
        ...
        [RavenTheory(RavenTestCategory.Indexes)]
        [RavenData(SearchEngineMode = RavenSearchEngineMode.All)]
        public void MapIndex_Any_And_All_ShouldWork(Options options)
        {
            var indexBuilder = new IndexDefinitionBuilder<DocWithLongs, object>
            {
                Map = docs => from doc in docs
                              ...
                              {
                                  doc.Id,
                                  HasBigNumbers = doc.ULongValues.Any(x => x > 100),
            ...
            var docs = new object[]
            {
                new DocWithLongs { ULongValues = [1UL, 200UL] }, // Any: true, All: true
                new DocWithLongs { ULongValues = [1UL, 2UL] }    // Any: false, All: true
            };
            ...
            AssertIndexBuilderRewritesAndRunsCorrectly(
                ...
                additionalRunAsserts: (store, indexName) =>
                {
                    using (var session = store.OpenSession())
                    {
                         var anyCount = session.Advanced
                            .RawQuery<dynamic>($"from index '{indexName}' where HasBigNumbers = true")
                            .Count();
                        Assert.Equal(1, anyCount);

                         var allCount = session.Advanced
                            .RawQuery<dynamic>($"from index '{indexName}' where AllPositive = true")
                            .Count();
